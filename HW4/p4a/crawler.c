...
int crawl(char *start_url,
  ...
  /*pthread_mutex_lock(&links_count_mutex);
  pthread_mutex_lock(&page_count_mutex);
  while(links_count != 0 || page_count != 0)
	  ;	
  pthread_mutex_unlock(&links_count_mutex);
  pthread_mutex_unlock(&page_count_mutex);*/
